...
The sealed-VM rung, on cella — hardware-isolated micro-VMs written directly on KVM, with no daemon, no capability, and no host network object. Each machine boots a dedicated guest kernel. The whole trial is one boot: a guest orchestrator runs the phases — setup, agent, collect, verify — writes each phase's result, and ends the machine with a forced reset. There is no channel into a live guest, and no exec-into.
