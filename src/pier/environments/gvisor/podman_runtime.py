...
Everything in :mod:`pier.environments.gvisor.runtime` that is genuinely
...
def assert_runtime_resolvable(
    ...
    """
    probe_name = f"pier-gvisor-runtime-probe-{uuid.uuid4().hex[:12]}"
    with tempfile.TemporaryDirectory(prefix="pier-gvisor-rootfs-") as rootfs:
        try:
