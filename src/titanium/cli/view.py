...
def view_command(
    ...
    Example usage:
        titanium view ./my-tasks          # auto-detects tasks
        titanium view ./my-jobs            # auto-detects jobs
        titanium view ./tasks --tasks      # force tasks mode
        titanium view ./jobs --port 9000
...
def _run_production_mode(
    ...
    *,
    ...
    from titanium.viewer import create_app
    ...
    console.print("[green]Starting Titanium Viewer[/green]")
...
def _run_dev_mode(
    ...
    folder_label = "Tasks folder" if mode == "tasks" else "Jobs folder"
    console.print("[green]Starting Titanium Viewer (dev mode)[/green]")
    console.print(f"  {folder_label}: {folder}")
    console.print(f"  Mode: {mode}")
    ...
    os.environ["TITANIUM_VIEWER_FOLDER"] = str(folder)
    os.environ["TITANIUM_VIEWER_MODE"] = mode

    # Run backend - uvicorn handles SIGINT/SIGTERM and shuts down gracefully
    try:
        uvicorn.run(
            "titanium.viewer:create_app_from_env",
            host=host,
            port=backend_port,
            log_level="info",
            reload=True,
