"""Podman environment for Titanium — no Docker API socket anywhere in the path.
...
Environment knobs (all optional):
    ...
    TITANIUM_PODMAN_BIN         podman binary            (default: podman)
    TITANIUM_PODMAN_COMPOSE     compose provider + args  (default: podman-compose)
    TITANIUM_PODMAN_IN_POD      --in-pod value           (default: false)
    TITANIUM_PODMAN_RUN_ARGS    extra args for podman run
    TITANIUM_PODMAN_CGROUP_FAIL_CLOSED
...
from titanium.environments.podman.podman_unix import PodmanUnixOps
...
def _which_compose(name: str) -> str | None:
    """PATH first (so TITANIUM_PODMAN_COMPOSE overrides behave normally), then the
    interpreter's bin dir — podman-compose is a titanium dependency, so it sits in
    ``.venv/bin``, which is not on PATH when titanium is invoked as
    ``.venv/bin/titanium``."""
...
class PodmanEnvironment(DockerEnvironment):
    ...
    def resource_capabilities(cls) -> EnvironmentResourceCapabilities:
        ...
        limit. Reporting False makes Titanium reject LIMIT/GUARANTEE tasks up front
    ...
    def _cgroup_controllers() -> tuple[bool, bool]:
        ...
        try:
            ...
            if os.environ.get("TITANIUM_PODMAN_CGROUP_FAIL_CLOSED", "").lower() in (
                "1",
                "true",
                "yes",
            ):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.podman_bin = os.environ.get("TITANIUM_PODMAN_BIN", "podman")
        self.compose_cmd = shlex.split(
            os.environ.get("TITANIUM_PODMAN_COMPOSE", "podman-compose")
        )
        self.compose_cmd[0] = _which_compose(self.compose_cmd[0]) or self.compose_cmd[0]
        self.in_pod = os.environ.get("TITANIUM_PODMAN_IN_POD", "false")
        self.run_args = os.environ.get("TITANIUM_PODMAN_RUN_ARGS", "")
    ...
    def _apply_selinux_relabel(self) -> None:
        ...
        relabel = os.environ.get("TITANIUM_PODMAN_SELINUX_RELABEL", "z")
    ...
    def preflight(cls) -> None:
        podman = os.environ.get("TITANIUM_PODMAN_BIN", "podman")
        compose = shlex.split(os.environ.get("TITANIUM_PODMAN_COMPOSE", "podman-compose"))
    ...
    async def _run_docker_compose_command(
        ...
        other flag Titanium uses is supported as-is.
