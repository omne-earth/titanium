...
from titanium.models.trial.config import ResourceMode, ServiceVolumeConfig
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._is_windows_container:
            ...
            from titanium.environments.docker.docker_windows import WindowsOps

            self._windows_container_name = f"titanium-{uuid.uuid4().hex[:12]}"
        ...
        if resolved_task_keys:
            titanium_keys = set(self._env_vars.to_env_dict(include_os_env=False).keys())
    ...
    def _effective_docker_image(self) -> str | None:
        ...
        ``TITANIUM_IMAGE_SOURCE=prebuilt`` opts into the upstream-parity behavior
        ...
        if not image:
            return None
        if os.environ.get("TITANIUM_IMAGE_SOURCE", "source") == "prebuilt":
            return image
    ...
    async def _run_docker_compose_command(
        ...
        if self._windows_container_name:
            env["TITANIUM_CONTAINER_NAME"] = self._windows_container_name
