...
def _staged_entry_name(index: int) -> str:
    ...
    """
    return f"{IN_ENTRY_PREFIX}{index:04d}"
...
def _place_entry(
    index: int, entry: GuestFile | GuestSymlink, root: str = BUILD_ROOT
) -> list[str]:
...
def _mkfs_script(*, size_bytes: int, boot_layer: BootLayer | None) -> str:
    ...
    lines = [
        ...
        f"tar -xpf /in/{IN_TAR_NAME} -C {BUILD_ROOT} --numeric-owner",
    ]
    if boot_layer is not None and boot_layer.entries:
        lines.append(_PLACEMENT_HELPERS.format(root=BUILD_ROOT))
        for index, entry in enumerate(boot_layer.entries):
            lines += _place_entry(index, entry, BUILD_ROOT)
    lines += [
        ...
        f"mkfs.ext4 -F -q -d {BUILD_ROOT} /out/rootfs.ext4",
    ]
    return "\n".join(lines)
...
def build_ext4(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="titanium-cella-mkfs-") as staging:
        ...
        staged_tar = staging_dir / IN_TAR_NAME
        try:
