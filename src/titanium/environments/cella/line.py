...
def router_policy_text() -> str:
    ...
    """
    return (
        "# The agent line's membranes -- one policy, both planes.\n"
        "# The wire side: the task peer reaching the proxy, and the\n"
        "# proxy replying. An incoming crossing is named by its\n"
        "# source (the task's address, an ephemeral port), so the\n"
        "# grant is the peer with a wildcard port, both ways.\n"
        "allow outgoing arp\n"
        "allow incoming arp\n"
        f"allow incoming {TASK_WIRE_ADDRESS}:*/tcp\n"
        f"allow outgoing {TASK_WIRE_ADDRESS}:*/tcp\n"
        "# The world side: DNS to the pinned resolver and 443 out.\n"
        "# Coarse on purpose -- the proxy holds the names, the\n"
        "# membrane holds the record.\n"
        f"allow outgoing {ROUTER_RESOLVER}:53/udp\n"
        f"allow incoming {ROUTER_RESOLVER}:53/udp\n"
        "allow outgoing *:443/tcp\n"
        "allow incoming *:443/tcp\n"
    )
...
def line_grants_text() -> str:
    ...
    return (
        ...
        "# proxy, and the wire plane's ARP. Nothing else.\n"
        "allow outgoing arp\n"
        "allow incoming arp\n"
        f"allow outgoing {ROUTER_WIRE_ADDRESS}:{PROXY_PORT}/tcp\n"
        f"allow incoming {ROUTER_WIRE_ADDRESS}:{PROXY_PORT}/tcp\n"
    )
