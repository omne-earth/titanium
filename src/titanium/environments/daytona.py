...
from titanium.environments.agent_setup import dockerfile_install_commands
...
class _DaytonaDirect(_DaytonaStrategy):
    async def start(self, force_build: bool) -> None:
        ...
        if snapshot_exists and env.agent_install_spec is not None:
            env.logger.warning(
                "Daytona snapshot is being used, so Titanium cannot inline the agent "
    ...
    def _infra_env_vars(self) -> dict[str, str]:
        env_vars: dict[str, str] = {
            ...
            "MAIN_IMAGE_NAME": _sanitize_docker_image_name(
                f"titanium__{self._env.environment_name}"
            ),
...
class DaytonaEnvironment(BaseEnvironment):
    ...
    async def _pin_resolved_hosts(self) -> None:
        ...
        command = (
            "cat >> /etc/hosts <<'EOF'\n"
            "# Titanium Daytona network allowlist host pins\n"
            f"{hosts}"
            "EOF"
        )
