...
from titanium.models.job.config import JobConfig, RetryConfig
...
from titanium.utils.env import sanitize_env_assignment
...
class TitaniumLockInfo(BaseModel):
...
class JobLock(BaseModel):
    ...
    titanium: TitaniumLockInfo = Field(default_factory=TitaniumLockInfo)
    ...
    def _canonical_payload(self) -> dict:
        payload = self.model_dump(
            mode="json",
            exclude={"created_at", "titanium", "invocation"},
        )
...
def _get_titanium_info() -> TitaniumLockInfo:
    return TitaniumLockInfo(
        version=_get_titanium_version(),
        git_commit_hash=_get_titanium_git_commit_hash(),
        is_editable=_get_titanium_is_editable_install(),
    )


def _get_titanium_version() -> str | None:
    try:
...
def _get_titanium_git_commit_hash() -> str | None:
    direct_url_data = _get_titanium_direct_url_data()
    ...
    if not _is_titanium_editable_install(direct_url_data):
        return None
...
def _get_titanium_is_editable_install() -> bool | None:
    direct_url_data = _get_titanium_direct_url_data()
    if direct_url_data is None:
        return None
    return _is_titanium_editable_install(direct_url_data)


def _get_titanium_direct_url_data() -> dict | None:
    try:
        dist = distribution("titanium")
    except PackageNotFoundError:
        return None
...
def _is_titanium_editable_install(direct_url_data: dict) -> bool:
