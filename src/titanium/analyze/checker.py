...
from titanium.analyze.backend import query_agent
from titanium.analyze.models import (
    QualityCheckResult,
...
import titanium.analyze

PROMPTS_DIR = Path(titanium.analyze.__file__).parent / "prompts"
...
async def run_check(
    ...
    if not task_paths.is_valid():
        raise ValueError(
            ...
            f"For trial analysis, use 'titanium analyze <trial-dir>' instead."
        )
