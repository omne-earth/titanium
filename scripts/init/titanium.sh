...
# 5. Root-provision the user-level container config directory. Owned by root
#    and not writable by titanium, so the user-level [engine.runtimes]
#    redirect documented as GVISOR-PODMAN.md §2.3's residual is closed
#    structurally: only root decides what "runsc" means for the runner.
RUNNER_HOME=$(getent passwd "$RUNNER" | cut -d: -f6)
sudo mkdir -p "$RUNNER_HOME/.config/containers"
sudo chown root:root "$RUNNER_HOME/.config" "$RUNNER_HOME/.config/containers"
...
echo "titanium provisioned — run trials with: make <target> RUNNER=titanium"
